...
SourceScout is original code, built as a local agent-native reach map for searching, reading, and verifying internet sources that are often difficult for agents to access reliably.
...
SourceScout does **not** vendor Agent-Reach code. It reimplements the useful pattern for Hermes:
...
SourceScout differs by making local coverage state, approval boundaries, and evidence requirements first-class.

## Adjacent agent capability systems

SourceScout is informed by, but does not vendor code from:
...
SourceScout routes to specialized engines instead of replacing them:
...
SourceScout also borrows ideas from harness-first agent systems:
...
SourceScout is licensed under BSD 3-Clause. Redistribution must preserve the copyright notice, license text, and this notice file when distributed with the source tree.
